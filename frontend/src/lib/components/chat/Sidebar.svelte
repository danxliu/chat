<script lang="ts">
	...
	async function handleDeletePersonalHistory() {
		if (!confirm('Are you sure you want to delete your personal history? This will erase all learned memories.')) return;
...
<div class="flex flex-col h-full border-r bg-muted/30">
	...
	<div class="p-4 flex flex-col gap-4">
		<Button onclick={createNewSession} class="w-full justify-start gap-2" variant="outline">
			<Plus class="h-4 w-4" />
			New Chat
		</Button>
	</div>
	...
	<div class="p-4 mt-auto border-t space-y-2">
		<Button variant="ghost" size="sm" class="w-full justify-start gap-2 text-muted-foreground" onclick={handleClearAllChats}>
			<Trash class="h-4 w-4" />
			Clear All Chats
		</Button>
		<Button variant="ghost" size="sm" class="w-full justify-start gap-2 text-muted-foreground" onclick={handleDeletePersonalHistory}>
