<script lang="ts">
	...
	const statusText = $derived(isThinking ? 'Thinking...' : 'View Thought Process');
</script>

<div class="mb-4 mt-2">
	<Collapsible.Root bind:open={isOpen} class="space-y-2">
		<Collapsible.Trigger class="flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground transition-colors">
			<Brain class="h-3 w-3" />
